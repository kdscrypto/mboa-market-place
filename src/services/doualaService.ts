...
export const fetchDoualaAds = async (limit: number = 12): Promise<Ad[]> => {
  try {
    ...
    // Filter for Douala ads only
    const doualaAds = adsData.filter((ad: any) => 
      ad.city && ad.city.toLowerCase() === 'douala'
    );

    console.log(`Found ${doualaAds.length} Douala ads out of ${adsData.length} total ads`);

    // Sort ads: Premium first (by creation date desc), then Standard (by creation date desc)
    const sortedAds = doualaAds.sort((a: any, b: any) => {
    ...
    console.log(`Successfully processed ${adsWithImages.length} Douala ads with images`);
    return adsWithImages;

  } catch (error) {
    console.error("Error in fetchDoualaAds:", error);
    throw error;
  }
};