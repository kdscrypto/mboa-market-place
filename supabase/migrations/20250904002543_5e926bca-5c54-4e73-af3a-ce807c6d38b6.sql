...
FOR UPDATE
USING (id = auth.uid())
WITH CHECK (id = auth.uid() AND role = get_user_role_safe(id));
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE a.id = p_ad_id 
      AND (a.user_id = p_user_id OR up.role IN ('admin', 'moderator'))
  ) THEN
    RETURN TRUE;
  END IF;
...
-- Create view for public ads without contact info
...
SELECT 
  ...
  CASE 
    WHEN auth.uid() IS NOT NULL AND public.can_view_contact_info(id, auth.uid()) 
    THEN phone 
    ELSE NULL 
  END as phone,
  CASE 
