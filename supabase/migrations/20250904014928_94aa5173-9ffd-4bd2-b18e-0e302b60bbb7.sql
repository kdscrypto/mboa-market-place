...
-- Ajouter un policy pour exclure les annonces supprimées/rejetées des listes publiques
-- Modifier le statut pour inclure 'deleted' comme statut possible
ALTER TABLE public.ads 
ADD CONSTRAINT valid_status CHECK (status IN ('pending', 'approved', 'rejected', 'deleted'));
...
BEGIN
  ...
  WHERE a.status = 'approved'
    AND a.status != 'deleted'  -- Exclure les annonces supprimées
  ORDER BY 
